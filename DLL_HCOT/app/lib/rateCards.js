...
var rateCards = (function () {
	...
	function fetchRateCards(_params) {
		...
		var _rcmLoggedIn = salesRep.get('rcmToken') && salesRep.get('rcmRefreshToken');
		var _vendorCode = salesRep.get('vendorCode');
		var _rcmVendorCode = salesRep.get('rcmVendorCode') || null;

		if (!_vendorCode || !_rcmLoggedIn) {
			_failureCallback && _failureCallback();
			return false;
		}

		if (!_rcmVendorCode) {
			_lastUpdate = new moment(0).toISOString();
			salesRep
				.set({
					'rcmVendorCode': _vendorCode,
					'lastRateCardUpdate': _lastUpdate
				})
				.save();
		}

		webservices.rateCardsRcm({
			...
			successCallback: function (_response) {
				...
				if (_response.success && _response.result.rateFactors) {
					...
					var ratePrograms = _response.result.ratePrograms;
					doLog && console.log('[rateFactors] - # of rate factors: ' + rateFactors.length || 0);
					doLog && console.log('[rateFactors] - # of rate programs: ' + ratePrograms.length || 0);
					...
					_rateCardsCollection
						...
						.add(_.map(rateFactors, function (_newRateCardInfo) {
							return {
								purchaseOption: _newRateCardInfo.purchaseOption,
								paymentFrequency: _newRateCardInfo.paymentFrequency,
								advancePayment: '' + _newRateCardInfo.advancePayments,
								advancePaymentType: '' + _newRateCardInfo.advancePaymentsType,
								promoCode: _newRateCardInfo.promoCode,
								term: _newRateCardInfo.term,
								min: '' + _newRateCardInfo.amountRangeMin,
								max: '' + _newRateCardInfo.amountRangeMax,
								paymentLevel: '' + _newRateCardInfo.paymentLevel,
								payments: '' + _newRateCardInfo.payments,
								points: '' + _newRateCardInfo.points,
								rateFactor: '' + _newRateCardInfo.value,
								lastUpdated: _today,
								interestRate: '' + _newRateCardInfo.interestRate,
								deferral: '' + _newRateCardInfo.deferral,
								versionId: _newRateCardInfo.versionId,
								vendorCodePoints: '' + _newRateCardInfo.vendorCodePoints,
								promoCodeName: _newRateCardInfo.rateProgram
							};
						}))
						.save();

					// Rate programs
					var _promosCollection = Alloy.createCollection('promo');
					var defaultValueCollection = Alloy.createCollection('defaultValue');
					var defaultValuesData = [];

					_promosCollection
						.wipe()
						.add(_.map(ratePrograms, function (_newPromoInfo, _index) {
							var description = _newPromoInfo.name || '';
							var program = _newPromoInfo.promoCode;
							var order = _newPromoInfo.order || _index;

							defaultValuesData.push({
								promoCode: program,
								term: _newPromoInfo.defaults && _newPromoInfo.defaults.term ? _newPromoInfo.defaults.term : null,
					...
					_successCallback && _successCallback({
						response: _response,
						salesRep: salesRep,
						promosCollection: _promosCollection,
	...
	function getAdvancePaymentTitle(_params) {
		...
		case 'S':
			...
			break;
		default:
			advancePaymentTitle += _params.advancePaymentType != null ? _params.advancePaymentType : ' in Advance';
		}
	...
	function getAllAdvancePayments() {
		...
		rateCards.fetch({
			query: {
				...
				order: 'advancePayment, advancePaymentType'
			}
		});
	...
	function getDefaultAdvancePayment() {
		...
		rateCard.fetch({
			query: {
				select: 'distinct advancePayment, advancePaymentType',
				limit: 1,
				order: 'advancePayment ASC, advancePaymentType ASC'
			}
		});
	 ...
	 * Will be empty (length = 0) if no valid rate card was found
	 */
	function getRateCardsForPromo(_paymentOption) {
	...
	function hasDefaultValues() {
		doLog && console.log(LOG_TAG, 'hasDefaultValues');
		var defaultValue = Alloy.createModel('defaultValue');
		var count = 0;
		...
		count = parser.parseToNumber(defaultValue.get('count'));

		doLog && console.log(LOG_TAG, 'hasDefaultValues() - ' + count);
	...
	function getTermsForPaymentOption(_paymentOption) {
		...
		if (_paymentOption) {
			...
			} else if (isActivePromo(promoCode)) {
