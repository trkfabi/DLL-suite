...
var permissionHelper = require('/helpers/permissionsHelper');
...
var application = (function () {
	...
	function splashWindowDone() {
		var createSessionValue = sessionManager.createSession({
			rcmLoggedInCallback: handleRcmLoggedIn
		});

		if (createSessionValue === Alloy.Globals.sessionStatus.rcmRequired) {
			return;
		}
		var isLoggedIn = sessionManager.isSessionSaved() && createSessionValue;
		if (!isLoggedIn) {
			attemptLogin();
		} else {
			initStates();
			...
			_.defer(function () {
				permissionHelper.requestPermissions({
					...
					failCallback: function () {
						appNavigation.showAlertMessage(L('storage_request_denied'));
					}
				});
			});
		}
	...
	/**
	 * @method handleRcmLoggedIn
	 * @private
	 * Handle the rcm login callback
	 * @return {void}
	 */
	function handleRcmLoggedIn() {
		doLog && console.log('[application] - handleRcmLoggedIn()');
		initStates();
		appNavigation.openMainWindow({
			stateChangeCallback: handleAppStateChange
		});
