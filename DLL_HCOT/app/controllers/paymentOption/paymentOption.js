...
$.refreshUI = function () {
	...
	if (paymentOption.isActive()) {
		...
		paymentRows.purchaseOptions.setActiveOptions(purchaseOptions);
		paymentRows.termOptions.setOptions(terms);
		paymentRows.paymentFrequency.setActiveOptions(frequencies);
		...
		if (!paymentOption.isSubmitted() && !hasShownAlert) {
			hasShownAlert = true;
...
$.initKeyboardToolbar = function () {
	paymentRows.rateFactor.initKeyboardToolbar();
};
/**
...
$.setSelected = function (_isSelected) {
	...
	if (OS_ANDROID) {
		$.numberCircle.visible = _isSelected;
	}
};
...
/**
 * @method setDuplicateButtonActive
 * Activates/Desactivates the duplicate button
 * @param {Boolean} _state `true` is the duplicate button should be active, `false` otherwise
 * @return {void}
 */
$.setDuplicateButtonActive = function (_state) {
	uiHelpers.setElementEnabled($.duplicateButton, _state);
};
...
function handlePaymentHeaderClick(_evt) {
	analytics.captureApm('[paymentOption] - handlePaymentHeaderClick() - ' + paymentOption.id);
...
function handlePaymentContainerSwipe(_evt) {
	...
	analytics.captureTiAnalytics('OTHC-MainWindow-SlidePaymentOptionRight');
