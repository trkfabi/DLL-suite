...
/**
 * @method updatePromoNameAnalytics
 * @private
 * Adds a counter to the promo name counter
 * @return {void}
 */
function updatePromoNameAnalytics() {
	doLog && console.log('[rateProgramPickerRow] - updatePromoNameAnalytics');
	paymentOption.addAnalyticsCounter('promoName');
}
...
$.refreshUI = function () {
	$.selectionLabel.text = stringFormatter.removeDefaultTerm(paymentOption.get('promoName'));
};
...
function handlePickerClick() {
	if (!pickerOpened) {
		...
		var options = promos.map(function (_promo) {
			return {
				title: stringFormatter.removeDefaultTerm(_promo.get('description')),
