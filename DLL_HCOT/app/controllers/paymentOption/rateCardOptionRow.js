...
function handleRateTypeBarClick(_evt) {
	var selectedPaymentOption = paymentOption.get('useRateCard');
	if (selectedPaymentOption === 0) {
		analytics.captureTiAnalytics('OTHC-MainWindow-SelectRateCard');
	} else {
		analytics.captureTiAnalytics('OTHC-MainWindow-SelectRateCardFactor');
	}
