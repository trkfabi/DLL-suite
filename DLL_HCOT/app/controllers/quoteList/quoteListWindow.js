...
$.refreshUI = function (_quotes, _options) {
	...
	// Change the optionsSelector to "Submitted" if the quote has been just submitted.
	...
	var showSubmitted = !!($.optionsSelector.getIndex() || 0);
	quoteIndexLoaded = 0;
	...
	quotesToShow = quotes.filter(function (_quote) {
		if (_quote.get('deleted')) {
			...
			return (showSubmitted === _quote.isSubmitted());
		}
	});
...
$.updateQuoteName = function (_quote) {
	...
	var alertDialogWindow = Alloy.createController('common/alertDialogWindow', alertDialogParams);
 ...
 * @return {void}
 */
function loadNextBatch(_count, _resetList) {
	...
	doLog && console.log(LOG_TAG, '- loadNextBatch - initial: ' + initial + ' final: ' + final);
	...
	if (_resetList) {
		$.quoteListSection.items = listItems;
	} else {
...
function handleQuoteListClick(_evt) {
	doLog && console.log(LOG_TAG, '- handleQuoteListClick quoteId: ' + _evt.itemId);
