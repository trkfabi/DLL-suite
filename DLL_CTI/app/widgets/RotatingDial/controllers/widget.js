...
var images = {
	...
	circularDialPressed	: $.circularDialPressed.toImage(),
...
exports.resetDial = function() {
	if (previousAngle !== 0) {
		...
		// $.circularDial.opacity = 0;
		// $.circularDialPressed.opacity = 1;
		$.circularDial.animate({
			transform : Ti.UI.create2DMatrix().rotate(angleInfo.degrees),
			duration : 250
		});
		$.circularDialPressed.animate({
			transform : Ti.UI.create2DMatrix().rotate(angleInfo.degrees),
			duration : 250
		}, function() {
			$.circularDial.opacity = 1;
			$.circularDialPressed.opacity = 0;
		});
	}
	resetTouchPoint();
};
...
exports.init = function () {
	$.centerTarget.addEventListener('touchstart', handleCenterTap);
	...
	sizes = {
		...
		circularDialPressed : {
			width : images.circularDialPressed.width / densityFactor,
			height : images.circularDialPressed.height / densityFactor
		},
		circularDial : {
...
function showDialTouched() {
	// $.circularDialPressed.animate({duration:20, opacity:1}, touchAnimationDone);
	$.circularDialPressed.opacity = 1;
	$.circularDial.opacity = 0;
};
function showDialUntouched() {
	// $.circularDial.animate({duration:50, opacity:1});
	// $.circularDialPressed.animate({duration:100, opacity:0});
	$.circularDial.opacity = 1;
	$.circularDialPressed.opacity = 0;
};
...
function updateDial(angleInfo) {
	$.circularDialPressed.transform = Ti.UI.create2DMatrix().rotate(angleInfo.degrees);
	$.circularDial.transform = Ti.UI.create2DMatrix().rotate(angleInfo.degrees);
