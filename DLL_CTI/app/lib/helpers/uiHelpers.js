 ...
 * @class Lib.helpers.uiHelpers
 * @singleton
 * @uses Lib.helpers.stringFormatter
...
var uiHelpers = (function () {
	...
	var expandCollapse = function (_params) {
		...
		if (container) {
			...
			doLog && console.log('[uiHelpers] - expandCollapse() - ' + (container.expanded ? 'expanded' : 'collapsed'));
	...
	var applyHintTextStyle = function (_textField) {
		if (OS_IOS) {
			...
			textField.attributedHintText = attr;
		}
	};
	...
	var createSearchButton = function () {
		return Ti.UI.createButton({
			systemButton: Ti.UI.iOS.SystemButton.SEARCH
		});
	};
	 ...
	 * @param {Boolean} _isSearch parameter, if true displays "Search" button instead of "Done".
	 ...
	 * @return {void}
	 */
	var initNumberFormatHandler = function (_textField, _pattern, _shouldRetainOnZero) {
		...
		_textField.addEventListener('change', function (_evt) {
			var _newVal = stringFormatter.cleanNonNumericString(_evt.source.value);
