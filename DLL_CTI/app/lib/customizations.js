...
var customizations = (function () {
	...
	Ti.App.addEventListener('app:creditLoaded', saveTermsAndConditions);
	...
	function saveArtifact(_artifact) {
		...
		analytics.captureApm('[customizations] - saveArtifact() - ' + _fileName);
	...
	function removeExpiredArtifacts() {
		...
		_.each(_expiredCustomizations, function (_expiredCustomization) {
			analytics.captureApm('[customizations] - removeExpiredArtifacts() - delete: ' + _expiredCustomization.get(
				'description'));
	...
	function loadTermsAndConditions() {
		analytics.captureApm('[customizations] - loadTermsAndConditions()');
	...
	function sendUpdateTermsEvent() {
		analytics.captureApm('[customizations] - sendUpdateTermsEvent()');
	...
	function saveTermsAndConditions(_evt) {
		analytics.captureApm('[customizations] - saveTermsAndConditions()');
	...
	function fetchCustomizations(_params) {
		_params = _params || {};
		analytics.captureApm('[customizations] - fetchCustomizations()');

		countryData = sessionManager.getCountryData();
		...
		webservices.customizations({
			lastRefreshDate: _lastUpdate.format(moment.defaultFormat),
			successCallback: function (_response) {
				...
				var _now = new moment().format(moment.defaultFormat);
				...
				if (_status.Code === 'S' && _artifacts.length > 0) {
					...
					salesRep.set('lastCustomizationsUpdate', _now).save();
	...
	function validateCustomizations(_valideAll) {
		analytics.captureApm('[customizations] - validateCustomizations()');
	...
	function getFile(_fileCode) {
		analytics.captureApm('[customizations] - getFile() - ' + _fileCode);
	 ...
	 */
	function getTerms() {
		analytics.captureApm('[customizations] - getTerms()');
	...
	function updateCustomizationsLanguage(_lang) {
		analytics.captureApm('[customizations] - updateCustomizationsLanguage() - ' + _lang);
