...
var joliCreator = function () {
		...
		typeValue: function (val) {
			if (!joli.getType(val)) {
				...
				if (joli.getType(val) === "string") {
					...
					// This line causes the symbol "$" to repeat itself in descriptions each time it is saved to the model
					// See BG-12164
					//val = val.replace(/\$/g, "$$$$");
	...
	joli.Connection = function (database, file) {
		...
		if (file) {
			this.database = Ti.Database.install(file, this.dbname);
		} else {
			this.database = Ti.Database.open(this.dbname);
		}
	...
	joli.query.prototype = {
		...
		getOperation: function () {
			...
			case 'insert_into':
				return 'insert into ' + this.data.from + ' (' + this.data.set.join(', ') + ') values (' + this.data.values.join(
					', ') + ')';
			case 'insert_replace':
				return 'insert or replace into ' + this.data.from + ' (' + this.data.set.join(', ') + ') values (' + this.data.values
					.join(', ') + ')';
			case 'replace':
			...
			default:
				throw (
					"Operation type Error. joli.query operation type must be an insert, a delete, a select, a replace or an update."
				);
			}
		},
		getQuery: function () {
		...
		hydrate: function (rows, hydratationMode) {
			...
			// TiSDK doesn't have method fieldCount()
