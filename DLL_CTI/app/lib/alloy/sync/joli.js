 ...
 * @version 1.0.0
 * @class Lib.alloy.Sync.joli
 * @singleton
 */

var Joli = require('/utils/joli');
...
var joliSync = (function () {
	...
	var beforeModelCreate = function (_config, _name) {
		...
		doLog && console.log('[joliSync] - beforeModelCreate() - ' + _name);
		...
		if (_table && _columns) {
			doLog && console.log('[joliSync] - beforeModelCreate() - ' + _name + ' has valid config.');

			_joli = Joli.connect(_dbName);

			if (!_adapter.idAttribute) {
				doLog && console.log('[joliSync] - No config.adapter.idAttribute specified for table "' + _table + '"');
			...
			if (_migrated) {
				...
				if (_freshVersion == null || _currentVersion >= _freshVersion) {
					...
					doLog && console.log('[joliSync] - migration _migrationCache: ' + JSON.stringify(_migrationCache));
	...
	var afterModelCreate = function (_model, _name) {
		...
		doLog && console.log('[joliSync] - afterModelCreate() - ' + _name);
	...
	var sync = function (_method, _model, _options) {
		...
		doLog && console.log('[joliSync] - sync() - ' + _table + ' - ' + _method);
		...
		case 'read':
			...
			if (_options.query) {
				...
				doLog && console.log('[joliSync] - _query.getQuery(): ' + _query.getQuery());
	 ...
	 * @param {Bakbone.Model} _model Model to save
	 * @param {utils.Joli.Connection} _joli Joli connection's instance to use to save the model
	 */
