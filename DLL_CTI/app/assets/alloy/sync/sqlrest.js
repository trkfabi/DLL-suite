...
function apiCall(_options, _callback) {
	// Ti.API.info("[sqlrest] - apiCall - Method : " + _options.method + " - localOnly : " + _options.localOnly);	
...
function Sync(method, model, opts) {
	...
	params.localOnly = model.config.localOnly || false; 
	...
	function createSQL(data) {
		...
		Ti.API.info("[sqlrest] - createSQL - Insert - length values : " + q.length);
...
module.exports.beforeModelCreate = function(config, name) {
	...
	if (Ti.Platform.osname === 'mobileweb' || typeof Ti.Database === 'undefined') {
		throw 'No support for Ti.Database in MobileWeb environment.';
	}
