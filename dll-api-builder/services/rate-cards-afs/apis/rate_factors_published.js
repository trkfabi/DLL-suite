...
const LOG_TAG = '[apis/rate_factors_published]';
...
const API = Arrow.API.extend({
	group: 'rate_factors',
	path: '/api/rate_cards/rate_factors/published',
	method: 'GET',
	...
	parameters: {
		...
		app: {
			description: 'App calling for ratefactors. Default afs',
			optional: true,
			type: 'query'
		}
	},
	action: function (req, resp, next) {
		...
		GatewayConnector
			...
			.then(vendorCode => {
				...
				return dataManager.getRateFactorsPublished(vendorCode, lastPublished, app);
			})
			.then(version => {
				if (version) {
