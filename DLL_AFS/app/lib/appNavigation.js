...
var appNavigation = (function () {
	...
	function closeMainWindow(_params) {
		if (controllers.mainWindow) {
			drawer && drawer.close();
			_.defer(function () {
				controllers.mainWindow && controllers.mainWindow.cleanUp && controllers.mainWindow.cleanUp();
				controllers.mainWindow = null;
			});
			_.defer(function () {
				controllers.customerWindow && controllers.customerWindow.cleanUp && controllers.customerWindow.cleanUp();
	...
	/**
	 * @method openCreditRatingSearchWindow
	 * Function that opens the credit rating search window
	 * @param {Object} Details for search credit rating window controller
	 * @return {void}
	 */
	function openCreditRatingSearchWindow(_params) {
		_params = _params || {};
		if (!controllers.creditRatingSearchWindow) {
			controllers.creditRatingSearchWindow = Alloy.createController('apple/creditRatingSearch/creditRatingSearchWindow',
				_params);
			controllers.creditRatingSearchWindow && openWindowInMainNav(controllers.creditRatingSearchWindow.window);
		}
	}

	/**
	 * @method closeCreditRatingSearchWindow
	 * Close search search credit rating window
	 * @param {Object} Details for the search credit rating window
	 * @return {void}
	 */
	function closeCreditRatingSearchWindow(_params) {
		if (controllers.creditRatingSearchWindow) {
			controllers.creditRatingSearchWindow && closeWindowInMainNav(controllers.creditRatingSearchWindow.window);
		}
		controllers.creditRatingSearchWindow = null;
	}
	...
	function handleQuoteNameLongpress(_params) {
		...
		if (controllers.mainWindow && quote && !quote.isSubmitted()) {
	...
	function openSignatureWindow(_params) {
		...
		if (!controllers.signatureWindow) {
			controllers.signatureWindow = Alloy.createController('authorization/signatureWindow', _params);
			controllers.signatureWindow.window && controllers.signatureWindow.window.open({
				modal: true
			});
		}
	};
	...
	function closeSignatureWindow(_params) {
		...
		if (controllers.signatureWindow) {
			controllers.signatureWindow.window && controllers.signatureWindow.window.close();
			controllers.signatureWindow.cleanUp && controllers.signatureWindow.cleanUp();
			controllers.signatureWindow = null;
		}
	};
	...
	function openSSNWindow(_params) {
		...
		if (!controllers.ssnWindow) {
			controllers.ssnWindow = Alloy.createController('authorization/ssnWindow', _params);
			controllers.ssnWindow.window && openWindowInMainNav(controllers.ssnWindow.window);
		}
	};
	...
	function closeSSNWindow(_params) {
		...
		if (controllers.ssnWindow) {
			controllers.ssnWindow.window && closeWindowInMainNav(controllers.ssnWindow.window, _params);
			controllers.ssnWindow.cleanUp && controllers.ssnWindow.cleanUp();
			controllers.ssnWindow = null;
		}
	};
	...
	function openDOBWindow(_params) {
		...
		if (!controllers.dobWindow) {
			controllers.dobWindow = Alloy.createController('authorization/dobWindow', _params);
			controllers.dobWindow.window && openWindowInMainNav(controllers.dobWindow.window);
		}
	};
	...
	function closeDOBWindow(_params) {
		...
		if (controllers.dobWindow) {
			controllers.dobWindow.window && closeWindowInMainNav(controllers.dobWindow.window, _params);
			controllers.dobWindow.cleanUp && controllers.dobWindow.cleanUp();
			controllers.dobWindow = null;
		}
	};
	...
	function openDriversLicenseWindow(_params) {
		...
		if (!controllers.driversLicenseWindow) {
			controllers.driversLicenseWindow = Alloy.createController('authorization/driversLicenseWindow', _params);
			controllers.driversLicenseWindow.window && openWindowInMainNav(controllers.driversLicenseWindow.window);
		}
	};
	...
	function closeDriversLicenseWindow(_params) {
		...
		if (controllers.driversLicenseWindow) {
			controllers.driversLicenseWindow.window && closeWindowInMainNav(controllers.driversLicenseWindow.window);
			controllers.driversLicenseWindow.cleanUp && controllers.driversLicenseWindow.cleanUp();
			controllers.driversLicenseWindow = null;
		}
	};
	...
	function closeTradeUpSearchWindow(_params) {
		if (controllers.tradeUpSearchWindow) {
			controllers.tradeUpSearchWindow.window && controllers.tradeUpSearchWindow.window.close();
	 ...
	 * @param {Function} _callback Function to run after tapping OK
	 * @return {void}
	 */
	function showAlertMessage(_message, _callback) {
		showAlert({
			message: _message,
			onClick: _callback
		});
	...
	function openWebViewWindow(_params) {
		doLog && console.log('[appNavigation] - openWebViewWindow');

		var params = _params || {};

		if (!controllers.webViewWindow) {
			controllers.webViewWindow = Alloy.createController('common/webViewWindow', params);
			if (OS_IOS) {
	...
	function closeWebViewWindow() {
		doLog && console.log('[appNavigation] - closeWebViewWindow');
	...
	return {
		...
		openCreditRatingSearchWindow: openCreditRatingSearchWindow,
		closeCreditRatingSearchWindow: closeCreditRatingSearchWindow
	};
})();
