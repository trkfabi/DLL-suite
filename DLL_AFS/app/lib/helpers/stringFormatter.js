...
var stringFormatter = (function () {
	...
	/**
	 * @method cleanAlphanumericString
	 * Removes all non-alphanumeric values from a string
	 * @param {String} _value string to clean
	 * @return {String} string without any non-alphanumeric characters
	 */
	function cleanAlphanumericString(_value) {
		_value = '' + (_value || '');

		return _value.replace(/([^\w- ])+/g, '');
	}
	 ...
	 * Converts a json object into a comma separated string for csv
	 * @param {Object} _jsonObject The json object
	 * @return {String}
	 */
	function convertJsonToCSV(_jsonObject) {
		var result = '';
		_.each(_jsonObject, function (_object) {
			var line = '';
			_.each(_object, function (_value) {
	...
	return {
		...
		cleanAlphanumericString: cleanAlphanumericString,
